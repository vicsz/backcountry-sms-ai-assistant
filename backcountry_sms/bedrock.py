...
EXTRACTION_SYSTEM_PROMPT = (
    ...
    "Treat a question about whether, when, or how to carry out an outdoor activity as weather when "
    "current weather would materially help answer it, even without words such as weather, forecast, "
    "rain, or wind. This includes questions about paddling, crossing open water, camping, shelter, "
    "layers, or whether conditions are suitable. Preserve the activity in activity and extract the "
    "requested time. For example, 'Can I safely cross this lake at noon?' is weather with activity "
    "open-water crossing and time_window noon; 'Should I put the tarp up before bed?' is weather with "
    "activity camping or shelter and an overnight time_window. Use the newest explicit history location only when "
    "CURRENT SMS has no named place or coordinates. If CURRENT SMS is a location-free follow-up to a prior "
    "weather exchange (for example, 'What about tomorrow?', 'Can I safely cross this lake at noon?', or "
    "'Should I put the tarp up before bed?'), classify it as weather and inherit that newest history "
    "location with location_source history. Deictic references such as 'this lake', 'here', or 'there' "
    "are not new locations; resolve them to the newest grounded history location. CURRENT SMS always wins. Preserve coordinates only "
...
ADVICE_SYSTEM_PROMPT = (
    ...
    "Answer weather-dependent outdoor decisions conditionally from the supplied facts: explain which "
    "weather factors support or limit the plan, but never guarantee that an activity is safe and never "
    "invent route, campsite, park-rule, water-temperature, or real-time-condition facts. State useful "
    "paddling/camping advice. Plain GSM-7 ASCII only, max 140 chars."
)
